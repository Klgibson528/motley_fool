...
def index(request):
    disclosure_html = disclosure(request)
    return render(request, 'index.html', {"disclosure": disclosure_html})
...
def disclosure(request):
    disclosure_api_file = os.path.join(
        PROJECT_PATH + '/apis/', 'disclosure_api.json')
...
def recommendations(request):
    disclosure_api_file = os.path.join(
        PROJECT_PATH + '/apis/', 'recommendations_api.json')
    with open(disclosure_api_file, "r") as f:
        disclosure_data = json.load(f)
    sleep(1.75)
    return JsonResponse(data=disclosure_data)
